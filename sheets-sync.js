...
(() => {
  ...
  async function loadGuideData(){
    ...
    const ranges=["Rutas!A2:O250","'Detalle del viaje'!A2:N150",`${config.placeGuideSheetName||"Lugares"}!A2:P250`,`${config.recommendationSheetName||"Recomendaciones"}!A2:J500`,"'Guía práctica'!A2:J250","Checklist!A2:K250","Notas!A2:F500","Reservas!A2:T200","Equipaje!A2:L100"];
    ...
    const [routeRows=[],detailRows=[],placeGuideRows=[],recommendationRows=[],practicalRows=[],checklistRows=[],noteRows=[],bookingRows=[],luggageRows=[]]=(data.valueRanges||[]).map(item=>item.values||[]);
    ...
    const guideData={routes,places,placeGuides,recommendations,practical,checklist,notes,bookings,luggage,loadedAt:new Date().toISOString()};
