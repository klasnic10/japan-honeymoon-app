...
(() => {
  ...
  async function loadGuideData(){
    ...
    const ranges=["Rutas!A2:O250","'Detalle del viaje'!A2:N150",`${config.placeGuideSheetName||"Lugares"}!A2:P250`,`${config.recommendationSheetName||"Recomendaciones"}!A2:J500`,"'Guía práctica'!A2:J250","Checklist!A2:K250","Notas!A2:F500","Reservas!A2:T200","Equipaje!A2:L100","Restaurantes!A2:S500"];
    ...
    const [routeRows=[],detailRows=[],placeGuideRows=[],recommendationRows=[],practicalRows=[],checklistRows=[],noteRows=[],bookingRows=[],luggageRows=[],restaurantRows=[]]=(data.valueRanges||[]).map(item=>item.values||[]);
    ...
    const checklist=checklistRows.filter(row=>row[0]).map((row,index)=>{const item={id:String(row[0]),phase:String(row[1]||""),category:String(row[2]||""),task:String(row[3]||""),owner:String(row[4]||"Común"),done:truthy(row[5]),notes:String(row[6]||""),dueDate:serialToIso(row[7]),priority:String(row[8]||""),dependency:String(row[9]||""),link:String(row[10]||"")};checklistRowById.set(item.id,index+2);return item;});
    noteRowById=new Map();
    ...
    const bookings=bookingRows.filter(row=>row[0]).map(row=>({id:String(row[0]),kind:String(row[1]||""),title:String(row[2]||""),detail:String(row[3]||""),startDate:serialToIso(row[4]),endDate:serialToIso(row[5]),origin:String(row[6]||""),destination:String(row[7]||""),status:String(row[8]||""),payment:String(row[9]||""),amount:Number(row[10])||0,currency:String(row[11]||""),time:String(row[12]||""),saleOpen:serialToIso(row[13]),cancellation:serialToIso(row[14]),locator:String(row[15]||""),link:String(row[16]||""),map:String(row[17]||""),alternative:String(row[18]||""),notes:String(row[19]||"")}));
    const luggage=luggageRows.filter(row=>row[0]).map(row=>({id:String(row[0]),actionDate:serialToIso(row[1]),item:String(row[2]||""),origin:String(row[3]||""),destination:String(row[4]||""),company:String(row[5]||""),status:String(row[6]||""),deliveryDate:serialToIso(row[7]),tracking:String(row[8]||""),carry:String(row[9]||""),confirmWith:String(row[10]||""),notes:String(row[11]||"")}));
    const restaurants=restaurantRows.filter(row=>row[0]).map(row=>({id:String(row[0]),routeId:String(row[1]||""),date:serialToIso(row[2]),day:String(row[3]||""),city:String(row[4]||""),area:String(row[5]||""),name:String(row[6]||""),type:String(row[7]||""),food:String(row[8]||""),address:String(row[9]||""),rating:Number(row[10])||0,reviews:String(row[11]||""),price:String(row[12]||""),priority:String(row[13]||""),reservation:String(row[14]||""),notes:String(row[15]||""),map:String(row[16]||""),source:String(row[17]||""),verified:serialToIso(row[18])||String(row[18]||"")}));
    const guideData={routes,places,placeGuides,recommendations,practical,checklist,notes,bookings,luggage,restaurants,restaurantsLoaded:true,loadedAt:new Date().toISOString()};
